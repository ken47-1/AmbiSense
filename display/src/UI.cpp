...
void UI::_updateDashboard(uint32_t timestamp,
    ...
    if (weather.valid) {
        /* Update Text & Icon */
        snprintf(buf, sizeof(buf), "%.0f\xC2\xB0""C", weather.temp);
        lv_label_set_text(_lblWeatherTemp, buf);
        lv_label_set_text(_lblWeatherIcon, getWeatherIcon(weather.weatherCode));

        /* Dynamic Icon Coloring */
        uint32_t iconCol;
        switch (weather.weatherCode) {
            case 0:  iconCol = theme->gold;       break; // Sunny
            case 1:
            case 2:  iconCol = theme->sky_blue;    break; // Partly Cloudy
            case 3:  iconCol = theme->dim;        break; // Overcast/Cloudy
            case 45:
            case 48: iconCol = theme->dim;        break; // Fog
            case 51: case 53: case 55:
            case 61: case 63: case 65:
            case 80: case 81: case 82: iconCol = theme->pastel_blue; break; // Rain
            case 71: case 73: case 75: iconCol = theme->pastel_blue; break; // Snow
            case 95:
            case 96:
            case 99: iconCol = theme->red;        break; // Stormy
            default: iconCol = theme->subtext;    break; // Unknown
        }
        lv_obj_set_style_text_color(_lblWeatherIcon, lv_color_hex(iconCol), 0);
        ...
        // The exact math from your snippet
        int sx = (VDIV_X - (iw + icon_gap + tw)) / 2;

        lv_obj_set_x(_lblWeatherIcon, sx);
        lv_obj_set_x(_lblWeatherTemp, sx + iw + icon_gap);

        /* Update the rest of the weather details */
        lv_label_set_text(_lblCondition, getWeatherDesc(weather.weatherCode));

        snprintf(buf, sizeof(buf), "Feels like: %.0f\xC2\xB0""C", weather.feelsLike);
        lv_label_set_text(_lblFeelsLike, buf);

        snprintf(buf, sizeof(buf), "%d%%", weather.humidity);
        lv_label_set_text(_lblHumidVal, buf);

        snprintf(buf, sizeof(buf), "%d hPa", weather.pressure);
        lv_label_set_text(_lblPressVal, buf);

        snprintf(buf, sizeof(buf), "%.0f km/h %s", weather.windSpeed, getWindDir(weather.windDirection));
        lv_label_set_text(_lblWindValDir, buf);
        
        lv_label_set_text(_lblSunriseVal, weather.sunrise);
        lv_label_set_text(_lblSunsetVal,  weather.sunset);

        /* --- Force Layout Recalculation --- */
		lv_obj_update_layout(_scrDashboard); 

		/* --- Re-center Row 5: Humidity + Pressure --- */
		{
			int iw = lv_obj_get_width(_lblHumidIcon);
			int hv = lv_obj_get_width(_lblHumidVal);
			int pw = lv_obj_get_width(_lblPressIcon);
			int pv = lv_obj_get_width(_lblPressVal);
			int total_w = iw + (5-6) + hv + (12-1) + pw + (5-1) + pv; // Using your exact gaps
		...
		/* --- Re-center Row 7: Sunrise + Sunset --- */
		{
			int ri = lv_obj_get_width(_lblSRIcon);
			int rv = lv_obj_get_width(_lblSunriseVal);
			int si = lv_obj_get_width(_lblSSIcon);
        ...
        lv_label_set_text(_lblWeatherTemp,  "--\xC2\xB0""C");

        lv_label_set_text(_lblWeatherIcon,  "\xF3\xB0\x8B\x97");
        lv_obj_set_style_text_color(_lblWeatherIcon, lv_color_hex(theme->subtext), 0);

        lv_label_set_text(_lblCondition,    "Unknown");
        lv_label_set_text(_lblFeelsLike,    "Feels like: --\xC2\xB0""C");

        lv_label_set_text(_lblHumidVal,     "--%");
        lv_label_set_text(_lblPressVal,     "---- hPa");

        lv_label_set_text(_lblWindValDir,   "-- km/h --");
        
        lv_label_set_text(_lblSunriseVal,   "--:--");
        lv_label_set_text(_lblSunsetVal,    "--:--");
    }
